...
var monitor = {
    init: function() {
      ...
      /**实时在线*/
      this.onlineCarInit = this.onlineCar();
      this.onlineCarInit.init();
  
      /**月注册车辆*/
      this.regMonitorContInit = this.regMonitorCont();
      this.regMonitorContInit.init();
      global.bind();
      global.appMove(1);

      /**新增接入注册数据*/
      this.regDataInit = this.regData();
      this.regDataInit.init();
      
    },

    /**新增接入注册数据*/
    regData:function(){
      var regTerminalCount = global.nums(7, '.allCarCont'),
      onlineVehicleCount = global.nums(7, '.onlineCarCont'),
      dayLineVehicleCount = global.nums(7, '.todayCarCont');
      var timer;

      var requAjax = function() {
        ZBase.CallServer.request("querySummary", null, function(data) {
          if (data.code == '10001') {
            var da = data.data;
            if (da) {
              // window.localStorage.regData = JSON.stringify(da);
              func(da);
            } else {
      ...
      var func = function(da) {             
        var allCarCont = da.regTerminalCount;
        var onlineCarCont = da.onlineVehicleCount;
        var todayCarCont = da.dayRegTerminalCount;
        regTerminalCount.init(allCarCont);
        onlineVehicleCount.init(onlineCarCont);
        dayLineVehicleCount.init(todayCarCont);
      };
    ...
    /**地图*/
    map: function() {
      var proviceTitle = 'regProvinceMap',
        cityTitle = 'regCityMap',
        provinces,
        citys,
        myChart,
        timer,
        regTerminalCount = global.nums(7, '.allCarCont'),
        onlineVehicleCount = global.nums(7, '.onlineCarCont'),
        dayLineVehicleCount = global.nums(7, '.todayCarCont');
      var pathUrl = 'image://assets/images/mapPoint.png';
      var mapOption = {
        legend: {
          orient: 'vertical',
          x: 'right',
      ...
      var geoCoordMap = {
        ...
        '潼南县': [105.841818, 30.189554],
        '铜梁县': [106.054948, 29.839944],
        '荣昌县': [105.594061, 29.403627],
        '璧山县': [106.231126, 29.593581],
        ...
        '中西区': [114.1543731, 22.28198083],
        '湾仔区': [114.1829153, 22.27638889],
        '东区': [114.2260031, 22.27969306],
        '南區': [114.1600117, 22.24589667],
        '油尖旺区': [114.1733317, 22.31170389],
        '深水埗区': [114.1632417, 22.33385417],
        '九龙城區': [114.1928467, 22.31251],
        '黄大仙区': [114.2038856, 22.33632056],
        '观塘区': [114.2140542, 22.32083778],
        '荃灣区': [114.1210792, 22.36830667],
        '屯门区': [113.9765742, 22.39384417],
        '元朗区': [114.0324381, 22.44142833],
        '北区': [114.1473639, 22.49610389],
        '大埔区': [114.1717431, 22.44565306],
        '西贡区': [114.264645, 22.31421306],
        '沙田区': [114.1953653, 22.37953167],
        '葵青区': [114.1393194, 22.36387667],
        '离岛区': [113.94612, 22.28640778],
      ...
      var initMap = function() {
        var name = mapOption.geo.map;
        if (!myChart) {
          myChart = echarts.init(document.getElementById('myMap'));
        } else {
          myChart.clear()
          if (name == 'china') {
            var dayLineMap = convertData(provinces[proviceTitle]);
          } else {
            var dayLineMap = convertData(citys[cityTitle][name]);
          }
        }
        mapOption.series[0].data = dayLineMap ? dayLineMap : [];
      ...
      var clickMap = function() {
        myChart.on('click', function(param) {
          console.info(param);
          if(param.name === currentName) return;

          currentName = param.name;

          if (param.componentType == 'series') {
            if (mapOption.geo.map == 'china') {
              if (param.name != '南海诸岛') {
                mapOption.geo.map = param.name;
          ...
          if(param.name == "海南") {
            var center = "琼中黎族苗族自治县";
            mapOption.geo.center = [geoCoordMap[center][0], geoCoordMap[center][1]];
            mapOption.geo.zoom = 8;
            
          } else {
            mapOption.geo.center = null;
            mapOption.geo.zoom = 1;
          }
      ...
      var clickTitle = function() {
        $('.map').on('click', '.car', function() {
          var title = $(this).attr('data-title');
          if(title == "dayReg") return;
          $('.allCar').find('.car').removeClass('active');
          $(this).addClass('active');
          cityTitle = title + 'CityMap';
          proviceTitle = title + 'ProvinceMap';
          initMap();
        });
      };
      ...
      var requAjax = function() {
        ZBase.CallServer.request("queryVehicleData", null, function(data) {
          if (data.code == '10001') {
            var da = data.data;
            if (da) {
              // window.localStorage.monitorMap = JSON.stringify(da);
              func(da);
            } else {
              /*var data = JSON.parse(window.localStorage.monitorMap);
              func(data);*/
      ...
      return {
        init: function() {
          ...
          requAjax();
          timer = setInterval(requAjax, 30 * 60 * 1000);
          clickMap();
          clickTitle();
        },
        resetBrand: function() {
    ...
    onlineCar: function() {
      ...
      var timer;
      var requAjax = function() {
        ZBase.CallServer.request("queryVehicleRealData", null, function(data) {
          if (data.code == '10001') {
            var da = data.data;
            if (da) {
              // window.localStorage.onlineCar = JSON.stringify(da);
              func(da);
            } else {
              /*var data = JSON.parse(window.onlineCar.regMonitorCont);
              func(data);*/
              console.log('返回为空');
            }
          }
      ...
      var func = function(data) {
        ...
        if (k >= middle) {
          var o = parseInt(k) - middle;
        } else {
          var o = parseInt(k) + middle;
        }
        $('.time').find('.start').html(data[len - 1].Hours + ':00');
        $('.time').find('.middle').html(o + ':00');
        $('.time').find('.end').html(k + ':00');
        data.reverse();
        var x1 = [];
        var y1 = [];
        var y2 = [];
        ...
        for (var i = 0; i < len; i++) {
          var p = data[i];
          var OnlineVehicleCount = p.count;
          x1.push(p.Hours + ':00');
          y1.push(p.runCount);
          y2.push(p.stopCount);
          y3.push(OnlineVehicleCount);
          min = min < parseInt(OnlineVehicleCount) ? min : parseInt(OnlineVehicleCount);
          max = max > parseInt(OnlineVehicleCount) ? max : parseInt(OnlineVehicleCount);
        }
        var widthY = 25;
        var widthQ = 16;
        switch (JSON.stringify(max).length) {
          case 1:
            widthY = -4;
            widthQ = 18.5;
            break;
        ...
        var left = widthY + (24 - data[len - 1].Hours) * widthQ;
        $('.shumiddle').css({ 'left': left });
        option.yAxis[0].min = parseInt(min / 1000) * 1000;
        option.xAxis[0].data = x1;
        option.series[0].data = y3;
      ...
      return {
        init: function() {
          myChart = echarts.init(document.getElementById('onlineCont'));
          myChart.setOption(option);
          requAjax();
          timer = setInterval(requAjax, 5 * 60 * 1000);
        },
        resetBrand: function() {
          clearTime();
          requAjax();
    ...
    regMonitorCont: function() {
      ...
      var timer;
      var requAjax = function() {
        ZBase.CallServer.request("queryVehicleMonthData", null, function(data) {
          if (data.code == '10001') {
            var da = data.data;
            if (da) {
              // window.localStorage.regMonitorCont = JSON.stringify(da);
              func(da);
            } else {
              /*var data = JSON.parse(window.localStorage.regMonitorCont);
              func(data);*/
              console.log('返回为空');
            }
          }
      ...
      var func = function(data) {
        ...
        for (var i = 0; i < len; i++) {
          ...
          if (p.opmonth) {
            name.push(p.opmonth);
            value.push(p.month_add_vehicle);
          }
        }
      ...
      return {
        init: function() {
          myChart = echarts.init(document.getElementById('regMonitorCont'));
          myChart.setOption(option);
          requAjax();
      ...
      //var myChart = echarts.init(document.getElementById('regMonitorCont'));
      myChart.setOption(option);
    },
    ...
    brand: function() {
      ...
      $('.brandNav').on('click', 'a', function() {
        ...
        _this.mapInit.resetBrand(); //地图
        _this.regMonitorContInit.resetBrand(); //月注册车辆数
        _this.onlineCarInit.resetBrand(); //实时在线
      });
    },
    ...
    carNum: function() {
      return;
      $('.monitor').addClass('om');
      var centralityStock = global.nums(7, '.centralityStock'),
        dealerStock = global.nums(7, '.dealerStock'),
        onTheWay = global.nums(7, '.onTheWay'),
        sale = global.nums(7, '.sale');
      var requAjax = function() {
        ZBase.CallServer.request("queryStockStateData", null, function(data) {
          if (data.code == '10001') {
            var da = data.data;
            if (da) {
              func(da);
      ...
      requAjax();
      var timer = setInterval(requAjax, 10 * 60 * 1000);
      var func = function(data) {
        var centralityStockNum = data.centralityStock,
          dealerStockNum = data.dealerStock,
          onTheWayNum = data.onTheWay,
          saleNum = data.sale;
        centralityStock.init(centralityStockNum);
        dealerStock.init(dealerStockNum);
        onTheWay.init(onTheWayNum);
        sale.init(saleNum);
      }
    }
  };